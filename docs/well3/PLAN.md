...
- Branches (last slice 8 mechanic): new exercise kind `branch`, a story fork. Content: prompt plus 2 to 5 paths, each a label plus a body, ids renumbered b1..bN like mapChoices. learnerItem sends labels only; the attempt response reveals the picked body, so the story stays a reveal and no new table is needed. A pick is never wrong (grade returns true for a real path, null otherwise), it logs an attempt and earns XP like any answer, and it stays out of spaced review because a story choice is not a fact to drill. Any pick counts toward lesson done, so a fork can never block learning. Guide side: the item editor authors forks, the public course preview shows labels but not bodies. Example: the musketeers course gains the oath fork at the Hotel de Treville. No migrations; no key means the fork still plays, grading aside.
