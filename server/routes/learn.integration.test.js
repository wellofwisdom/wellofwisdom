...
async function seedCourse(db, familyId) {
  const c = await db.query("insert into courses (family_id, title, topic, status, learner_id) values ($1,$2,$3,'published',null) returning id", [familyId, `Course ${Date.now()}`, "fractions"]);
  ...
  const num = await db.query("insert into lesson_items (lesson_id, type, position, content) values ($1,'exercise',1,$2) returning id", [lid, JSON.stringify({ prompt: "Half of 10?", kind: "numeric", answer: 5 })]);
  return { courseId, lessonId: lid, mcqId: Number(mcq.rows[0].id), numId: Number(num.rows[0].id) };
}
describe("learn integration", () => {
  ...
  it("attempt graded server-side and answer never in learner tree", async () => {
    ...
    const { courseId, mcqId } = await seedCourse(db, fam.familyId);
  ...
  it("learner cannot see another family course", async () => {
    ...
    const { courseId } = await seedCourse(db, famA.familyId);
