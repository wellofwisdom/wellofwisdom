...
/** Illustrate the world: encounters, chapter covers, character portraits.
 *  Costs money per image, so it is always an explicit request and it reports
 *  how many are waiting before it starts. */
router.post("/:adventureId/art", auth.requirePerm("spend_media"), async (req, res, next) => {
  try {
    const adventureId = num(req.params.adventureId);
    const adv = await ownedAdventure(adventureId, req.user.familyId);
    if (!adv) return bad(res, "not_found", 404);
    ...
    // One count across all three kinds, so the button's number and the job's
    // work list cannot disagree. Only approved characters: an unapproved
    // invention is not in the world yet.
    const { chaptersNeedingArt } = require("../lib/questgen");
    const [pendingEncounters, pendingCharacters] = await Promise.all([
      db.query(
      ...
      db.query(
        `select count(*)::int as n from adventure_characters
          where adventure_id = $1 and portrait_url is null and approved`,
        [adventureId]
      ),
    ]);
    const pending =
      pendingEncounters.rows[0].n + chaptersNeedingArt(adv.world).length + pendingCharacters.rows[0].n;
    if (!pending) return bad(res, "nothing_to_draw");

    const jobs = require("../lib/jobs");
    const jobId = await jobs.enqueue(
      req.user.familyId, "world-art", { adventureId, created_by: req.user.id }, req.user.id
    );
    res.status(202).json({ jobId, pending });
  } catch (err) {
    next(err);
  }
});
